		...
		for (int i = 0; i < num_iter; i++) {
			...
			for (train.data->begin(); !train.data->end(); train.data->next()) {
				...
				if (task == 0) {
					...
					/*
					train.target(train.data->getRowIndex()) : y
					p : y_predict
					*/
					double tx_temp = tx;
					if (train.target(train.data->getRowIndex()) + 1.0 <= 0.000000001){
						tx_temp = 1.0;
					}
					if (p * train.target(train.data->getRowIndex()) >= tx_temp){
						mult = 0;
					}
					else{
						if ((p * train.target(train.data->getRowIndex())) + slope <= 0.000000001){
							mult = 0;
						}
						else{
							mult = -train.target(train.data->getRowIndex());// mult = -y,L对θ的偏导等于 mult *(y_predict对θ偏导)
						}
