...
int main(int argc, char **argv) {
	...
	try {
		...
		const std::string param_tx = cmdline.registerParameter("tx", "tx for SGD & SGDA; default=1"); //MODIFY BY ZXK 17/11/27 新增参数 tx
		const std::string param_slope = cmdline.registerParameter("slope", "slope for SGD & SGDA; default=1");//MODIFY BY ZXK 17/11/27 新增参数 slope
		...
		if (!cmdline.hasParameter(param_tx)) { cmdline.setValue(param_tx, "0.1"); }
		...
		if (cmdline.hasParameter(param_val_file)) {
			if (cmdline.getValue(param_method).compare("sgda")) {
				...
				std::cout << "WARNING: lamda ratio is only used for SGDA." << std::endl;
			}
			else {
		...
		if (!cmdline.getValue(param_method).compare("sgd")) {
			...
			((fm_learn_sgd_element*)fml)->tx = cmdline.getValue(param_tx, 1.0);//MODIFY BY ZXK 17/11/27 
			((fm_learn_sgd_element*)fml)->slope = cmdline.getValue(param_slope,1.0);
			...
			((fm_learn_sgd_element_adapt_reg*)fml)->tx = cmdline.getValue(param_tx, 1.0);//MODIFY BY ZXK 17/11/27
			((fm_learn_sgd_element_adapt_reg*)fml)->slope = cmdline.getValue(param_slope, 1.0);
		}
